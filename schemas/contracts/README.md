...
- C10 is the S10 runtime wire surface for tokens, policy, quota, launch,
  sandbox, audit, and checkpoint-signing payloads. C10 v5 adds required signed
  soft/hard exfiltration byte thresholds; policy and checkpoint signatures stay
  on the HMAC-shaped `Signature` type while budget and scope tokens use
  `TokenSignature`, allowing Ed25519 public-key verification.
