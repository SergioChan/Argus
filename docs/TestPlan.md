...
| S11-TC39 | integration | **Observatory v0 static render + tamper banner.** Signed C3 v1.1 report + its C4 lineage; render → self-contained static page showing six-check verdicts, perturbation pairs, insensitivity flags, tier justification, referee identity, provenance chain, signature re-verified via argusverify; then flip one byte of the report and re-render. | rendered verdicts/tier byte-match report fields; tampered input → explicit FAIL banner and nonzero exit; no silent page. |
