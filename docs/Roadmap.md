...
7. **The Argus Observatory (Rev B.1) is the product's user-facing surface, staged with the trust plane it renders.** Three S11 subtasks (PRD S11 FR-29/30/31; TestPlan S11-TC39/40/41): **S11-T36 Observatory v0 [M1]** — a self-contained static page rendering one verified run (signed C3 v1.1 report + C4 lineage: six-check verdicts, perturbation pairs, tier justification, referee identity, provenance chain), signature re-verified offline via argusverify, tampered inputs render a FAIL banner; it is the **M1.5 pilot-demo artifact**. **S11-T37 Observatory v1 [M2]** — read-only live pipeline view over the platform semantic events (intake→build→adapter→verify→report + ledger-append stream). **S11-T38 Observatory v2 [M5]** — the debate arena: renders the C4 DebateLedger (proponent/challengers/referee rounds, verdicts, fitness only from signature-valid scores, killed-spurious counter). The Observatory is strictly read-only (obs.read) and never on the trust path — it *shows* verify-before-trust; it never *grants* it.
