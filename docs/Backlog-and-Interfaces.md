...
| C3 | S3 | Verifier Interface + Validation Report (**current v2.0; v1.1 compatibility baseline**) | list_profiles/verify/challenge; the **only** admissible source of tier > ran-toy and the **only** admissible recursion reward. **v1.1** added the 6 ValidationReport fields for adversarial red-blue debate: `perturbation_pairs`, `insensitivity_flags`, `challenger_panel`, `independence_attestation_debate`, `referee`, `debate_ref`; **v2.0** makes the trust-surface fields schema-required and gate-enforced for Observatory VERIFIED semantics |
...
| S11-T36 | Observatory v0: static verified-run report page | M | - | C3 report render, C4 lineage read, argusverify | S11-TC39 passes: signed current C3 report + C4 lineage render to a self-contained static HTML page (six-check verdicts, perturbation pairs, insensitivity flags, tier justification, referee identity, provenance chain); report signature re-verified offline via argusverify at render time; the VERIFIED banner is allowed only when signature, content hash, lineage, tier coupling, schema validation, every six-check status, bidirectional perturbation verdicts, empty insensitivity flags, and referee distinctness all pass; tampered report, invalid lineage, or semantic gate failure renders an explicit FAIL banner and nonzero CLI exit, never a silent page; the M1.5 pilot-demo artifact |
...
- **C3** produced by S3 (S3-T01.., current **v2.0** with **v1.1** retained as the compatibility baseline); consumed with evidence by S1, S2, S4, S5, S7, S8, S9, S11, S12. (**S7 added** — extrapolation-flag reciprocity, §3.2. S4 is the heaviest debate-field consumer via the debate loop; S8/S9 consume the ValidationReport debate fields.)
...
| C3 Verifier Interface + Validation Report (**current v2.0; v1.1 compatibility baseline**) | S3 | S5,S2,S4,S1,S7,S9,S11,S12,S8 | all present with subtasks | **consistent** | **v1.1** adds the 6 ValidationReport debate fields (`perturbation_pairs`, `insensitivity_flags`, `challenger_panel`, `independence_attestation_debate`, `referee`, `debate_ref`), produced by S3-TPR1..TPR5 and consumed heavily by S4 (debate loop), S8 (tier-coupling), S9 (emission gate); **v2.0** makes the Observatory/trust-surface fields required and fail-closed in the VERIFIED gate. **FIX 5 (reciprocity):** S7 added to C3's acknowledged consumers — S7 emits the `extrapolation_flag` in its C6 result and S3 profiles consume it, setting the affected check to INCONCLUSIVE (see §3.2). |
...
| DebateLedger | **Produced by S4** (S4-TDB5) as an append-only **C4** provenance record of all ChallengeRounds for an artifact; `debate_ref` in the current C3 ValidationReport points into it. Declared consumers: **S3** (referee reads history), **S8** (C4 storage/lineage), **S9** (evidence bundle). | **consistent** | Producer + all three consumers have subtask evidence; coupling is C4-mediated. |
...
4. **The C3 adversarial red-blue debate loop couples S4 (evolver/proponent) → S3 (referee) → S8/S9 (provenance + emission) across four contracts and must stay bit-consistent.** The self-improvement loop is now a multi-agent adversarial peer review: S4 produces the proponent candidate and runs the debate (S4-TDB1..TDB6), an independent challenger panel (≥K, lineage-disjoint via C5) attacks it with must-react/must-not-react probes, and the S3 referee (≠ proponent, signed, non-gameable) adjudicates via ChallengeVerdict — requiring `must_react_pass AND must_not_react_pass AND NOT insensitivity_detected`. The current C3 ValidationReport (v2.0, with v1.1 as the compatibility baseline) is the sole channel carrying this verdict through `perturbation_pairs`, `insensitivity_flags`, `challenger_panel`, `independence_attestation_debate`, `referee`, and `debate_ref`, and the DebateLedger (S4→C4) is the provenance spine for it. **Risk:** if S3's insensitivity detector (S3-TPR3) or the referee-≠-proponent enforcement (S3-TPR4) diverges from S4's admission logic, a planted-spurious model that ignores the data (insensitive) can survive to the human gate, or a valid claim can be wrongly killed. **Mitigation:** retain the C3 v1.1 M0 compatibility baseline while enforcing the current v2.0 semantic gates; make the insensitivity-catch and referee-separation KPIs hard 100% gates; require the challenger panel to be refreshed and lineage-disjoint each round (S3-TPR5, S4-TDB2, S4-TDB4) so proponent overfit and challenger collusion are both caught.
