...
| FR-29 | Observatory v0: static verified-run report page | P1 | Render one verified run as a self-contained static HTML page from its signed current C3 report and C4 lineage: six-check verdicts, perturbation pairs, insensitivity flags, claim tier + justification, referee identity (distinct_from_proponent), and the provenance chain. The report signature is re-verified offline via the shared argusverify library at render time; VERIFIED is allowed only when schema validation, every check status, bidirectional perturbation verdicts, empty insensitivity flags, and referee distinctness all pass; a tampered report, invalid lineage, or semantic gate failure renders an explicit FAIL banner, never a silent page. This is the M1.5 pilot-demo artifact — the first thing a pilot physicist sees. |
